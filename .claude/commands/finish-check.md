...
- [ ] **绑定变更可恢复**：删除/替换 SSL 绑定前是否先**捕获旧绑定完整参数**（优先 httpapi 结构化查询含高级 SSL 参数，查询失败降级 `netsh show` 最小三字段）？操作后是否明确区分目标生效、确认不存在、异常绑定与查询失败：确认不存在时直接恢复，异常绑定先删后恢复，状态未知时禁止破坏性回滚？恢复后是否再次查询验证旧证书哈希，而非信任命令输出？
